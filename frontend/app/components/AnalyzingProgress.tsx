    ...
    <div className="card card-border bg-base-100 shadow-sm animate-fade-up">
      <div className="card-body items-center text-center gap-8 py-10">
        <div className="space-y-1">
          ...
          <p className="text-xs text-base-content/60 font-mono uppercase tracking-widest">
            {overallPct}% complete
          </p>
        </div>
        ...
        {activeEntry && (
          <div className="w-full max-w-lg mt-2">
            <p className="text-xs font-mono mb-4 text-base-content/75 truncate px-4">
              <span className="opacity-50 mr-2">CURRENT:</span>
              {activeEntry[0].replace(/\.mp3$/i, "")}
            </p>
            <ul className="steps steps-horizontal w-full text-xs font-mono tracking-widest uppercase">
              {STEP_ORDER.map((step) => {
        ...
        {completedResults.length > 0 && (
          <div className="divider text-xs font-mono uppercase tracking-widest text-base-content/45 w-full mt-6">
            Log
          </div>
        )}
