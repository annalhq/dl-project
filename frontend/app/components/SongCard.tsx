        ...
        <div className="card-body p-4">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-error mt-0.5 opacity-80">{result.error}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-8 h-8 rounded bg-base-content/5 text-base-content/50 flex items-center justify-center flex-shrink-0">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 19.5V15m6 4.5V15M9 15v-3.75a3 3 0 013-3v0a3 3 0 013 3V15M9 15h6" />
              </svg>
            </div>
            <div className="min-w-0">
              <p className="font-medium text-sm truncate card-title tracking-tight" title={name}>
                {name}
              </p>
              <p className="font-mono text-[10px] uppercase tracking-widest text-base-content/40 mt-1">{result.filename}</p>
            </div>
          </div>
          <span
            ...
            <div className="space-y-2 pt-1 pb-2">
              {sortedProbs.map(([g, v]) => (
                <div key={g} className="flex items-center gap-2 font-mono text-xs">
                  <span className="w-[72px] text-right uppercase text-base-content/50 tracking-widest text-[10px]">
                    {g}
                  </span>
                  <progress
                  ...
                  ></progress>
                  <span
                    className={`w-10 text-right tracking-widest text-[10px] ${
                      g === result.genre
